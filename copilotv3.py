...
<style>
    ...
    .tab-title {font-size: 1.5em; font-weight: bold; margin: 1rem 0;}
</style>
...
# -------------------------------
# NSE API accepts stock symbols without the .NS suffix.
FO_STOCKS = [
...
def get_nse_session():
    ...
    Priming the session with a GET request to NSE homepage is necessary to obtain cookies.
    """
    ...
    try:
        # Prime the session to get cookies
...
def fetch_nse_quote(symbol, session):
    """Fetch real-time quote for a given symbol from the NSE API, with fallback simulation."""
    try:
...
def fetch_nse_indices():
    """Fetch NSE indices data (specifically NIFTY 50) from the NSE API, with simulation fallback."""
...
def fetch_live_fo_data():
    """Fetch live F&O data via the NSE API using multi-threaded calls for speed."""
    session = get_nse_session()
    fo_data = {}
...
def create_heatmap_from_ui(ui_heatmap):
    ...
    fig.update_layout(title="F&O Stocks Heatmap", xaxis=dict(showticklabels=False), yaxis=dict(showticklabels=False), height=400)
    return fig
...
def create_sector_bar_chart(ui_sector):
    ...
    fig = go.Figure(data=[go.Bar(y=sectors, x=values, orientation='h', marker_color=colors, text=[f"{v}" for v in values], textposition='auto')])
    fig.update_layout(title="Sector Performance", xaxis_title="Average Change %", height=400)
    return fig

# -------------------------------
# Build the App UI Using Tabs
# -------------------------------
def main():
    # Header area using UI config
    ...
    # Create tab-based navigation using mainTabs list from ui_config
    tabs = st.tabs(ui_config["mainTabs"])
    
    # Tab 1: Charts
    with tabs[0]:
        st.markdown(f"<div class='tab-title'>Charts</div>", unsafe_allow_html=True)
    ...
    # Tab 2: Scans
    with tabs[1]:
        st.markdown(f"<div class='tab-title'>Scans</div>", unsafe_allow_html=True)
    ...
    # Tab 3: Dashboard
    with tabs[2]:
        st.markdown(f"<div class='tab-title'>Dashboard</div>", unsafe_allow_html=True)
        dashboard = ui_config["components"]["dashboard"]["metrics"]
        cols = st.columns(len(dashboard))
    ...
    # Tab 4: Heatmap
    with tabs[3]:
        st.markdown(f"<div class='tab-title'>Heatmap</div>", unsafe_allow_html=True)
        heatmap_fig = create_heatmap_from_ui(ui_config["components"]["heatmap"])
        if heatmap_fig:
            st.plotly_chart(heatmap_fig, use_container_width=True)
    
    # Tab 5: Advance/Decline
    with tabs[4]:
        st.markdown(f"<div class='tab-title'>Advance/Decline</div>", unsafe_allow_html=True)
        ...
        fig.update_layout(title=f"Advance/Decline: {adv_dec['advancers']}/{adv_dec['decliners']}", height=300)
        st.plotly_chart(fig, use_container_width=True)
    
    # Tab 6: Sectors
    with tabs[5]:
        st.markdown(f"<div class='tab-title'>Sectors</div>", unsafe_allow_html=True)
