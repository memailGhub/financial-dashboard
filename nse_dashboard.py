        ...
        for j in range(cols):
            if symbols_grid[i, j]:
                ...
                if symbol_name in fo_data:
                    ...
                    row_text.append(f"{symbol_name}<br>Rs{price:.1f}<br>{change_val:+.1f}%")
                else:
                    row_text.append("")
            else:
                row_text.append("")
...
def main():
    ...
    st.markdown('<h1 class="main-header">India NSE F&O Dashboard - LIVE</h1>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div style="text-align: center; color: #666; margin: 10px 0;">Live updates every 30 seconds | Last updated: {datetime.now().strftime("%H:%M:%S")}</div>', unsafe_allow_html=True)
        ...
        if fo_data:
            ...
            with scan_col:
                ...
                for symbol, data in sorted_stocks[:5]:
                    ...
                    if change_pct >= 0:
                        st.markdown(f'<span class="stock-gain">{symbol} Rs{price:.1f} +{change_pct:.1f}% @{timestamp}</span>', unsafe_allow_html=True)
                    else:
                        st.markdown(f'<span class="stock-loss">{symbol} Rs{price:.1f} {change_pct:.1f}% @{timestamp}</span>', unsafe_allow_html=True)
            ...
            with advdec_col:
                ...
                fig.update_layout(
                    title=f"Market Breadth: {advancing} UP {declining} DOWN {unchanged} FLAT",
                    height=300,
                    plot_bgcolor='rgba(0,0,0,0)'
                )
            ...
            st.markdown(f"*LIVE DATA | Last updated: {current_time} | Auto-refresh: 30s*")
            st.markdown("*Data source: NSE India (Real-time)*")
            ...
            st.error("Failed to fetch live market data. Please check your connection and try again.")
            if st.button("Retry"):
                st.rerun()
